...
const Arquivos = ({ userId, nome, logo, setLogo }) => {
  ...
  const sendFile = async () => {
    ...
    const response = await fetch(`https://casa-cor.herokuapp.com/profissional/fileUpload`, {
      method: 'POST',
      body: dataForm,
    });
  ...
  const removerFile = async (logo, userId) => {
    const response = await api.post(`/profissional/fileDelete`, {
      logoId: logo,
      userId: userId,
    });
    setLogo(null);
        ...
        {logo ? (
          ...
          >
            <img
              ...
              src={`https://casa-cor.herokuapp.com/images/profissionais/${logo}.jpg`}
              alt="BigCo Inc. logo"
            />
            <button
